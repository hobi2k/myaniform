...
def patch_char_generate(description: str) -> dict:
    """캐릭터 첫 이미지 — SDXL 워크플로우로 고품질 생성."""
    return patch_image(description, workflow="sdxl")


def patch_character_sheet(character_name: str, description: str) -> dict:
...
def patch_voice_design(voice_design_text: str, sample_text: str = "안녕하세요.") -> dict:
    wf = load_workflow("ws_voice_design.json")
    for node in _iter_nodes(wf):
        cls = node.get("class_type", "")
        inp = node.setdefault("inputs", {})
