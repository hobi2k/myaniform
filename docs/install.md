...
# 2) 리포 클론 (ComfyUI 와 custom_nodes 는 벤더링 포함)
git clone https://github.com/hobi2k/myaniform.git
cd myaniform

# 3) 파이썬 venv + 의존성 + sageattention
bash setup.sh
...
- `ComfyUI/custom_nodes/*` — 17개 커스텀 노드: WanVideoWrapper, MMAudio, FishAudioS2, FL-Qwen3TTS, VFI, VHS, GGUF, IPAdapter_plus, KJNodes, Easy-Use, rgthree, Crystools, Geeky_AudioMixer + **Impact-Pack / Impact-Subpack** (SDXL FaceDetailer) + **vnccs / vnccs-utils** (VNCCS 캐릭터 시트·스프라이트)
...
3. **수동 재시도**:
   ...
   ```
4. **정 안 되면** — `ws_loop.json` / `ws_effect.json` 의 `attention_mode` 를 `sdpa` 로 바꿔 임시 우회. 단 블록 스왑만으로는 OOM 날 확률이 높으므로 블록 수를 더 늘려야 함 (`blocks_to_swap: 30~40`).
...
| `CIVITAI_TOKEN` | Dasiwa Illustrious SDXL, SmoothMix LoRA | civitai.com → Settings → API Keys |
...
토큰 없이도 대부분 공개 모델은 받아진다. 스킵된 모델은 스크립트 끝에 요약됨.
...
| Qwen3-TTS (gated) | `tts/Qwen3TTS/` | 4 variants | 16GB |
...
`workflows/ws_tts_clone.json` 의 `FL_Qwen3TTS_VoiceClone` 노드에서 **`x_vector_only_mode` 가 반드시 `true`**. 자세한 사유는 [memory file](../../../.claude/projects/-home-hosung-pytorch-demo/memory/project_tts_x_vector_mode.md) 또는 operations.md 2항 참고.
...
```
myaniform/
├── ComfyUI/                     # 벤더링 (git clone 에 포함)
│   ├── custom_nodes/            # 13개 커스텀 노드 벤더링
